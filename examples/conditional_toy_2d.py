...
from flowcon.datasets.base import InfiniteLoader, PlaneDataset, load_plane_dataset
...
def main():
    ...
    for i in range(num_iter):
        ...
        optimizer.zero_grad()
        loss = -flow.log_prob(x=x, context=y).mean()
        loss.backward()
        optimizer.step()
        scheduler.step()
        if i % 50 == 0:
            logger.info(f"iteration {i + 1}; Loss: {loss.item():.2e}")

        if (i + 1) % 100 == 0:
            plot_model(flow, train_dataset)


def build_flow(num_layers=5, num_shared_embedding=50):
    base_dist = DiagonalNormal(dim=2)
    ...
    densenet_factory.set_logabsdet_estimator(
        brute_force=True,  # set this to false for high dimensions (>3)
    ...
    flow = Flow(transform, base_dist, context_embedder=context_embedder)
...
def plot_model(flow: Flow, dataset: PlaneDataset):
    ...
    ax = ax.flatten()
    xline = torch.linspace(-4.0, 4.0, 100).to(device)
    yline = torch.linspace(-4.0, 4.0, 100).to(device)
