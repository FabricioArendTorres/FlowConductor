...
@parameterized_class(
    ("batch_size", "context_features", "features", "lipschitz_constant", "use_residual_blocks"),
    [
        (10, 2, 1, None, True),
        (2, 2, 4, None, True),
        (10, 2, 2, None, True),
...
class ConditionalSVDTest(ConditionalTransformTest):
    def setUp(self):
        ...
        self.transform = ConditionalSVDTransform(
    ...
    def test_matrix_properties(self):
        ...
        householder_U, diag_entries_S, householder_Vt, bias = self.transform._get_matrices(
            conditional_params
        )
        householder_U_matr, householder_Vt_matr = householder_U.matrix(), householder_Vt.matrix()

        self.assert_tensor_is_good(
            householder_U_matr, shape=[self.batch_size, self.features, self.features]
        )
        self.assert_tensor_is_good(
            householder_Vt_matr, shape=[self.batch_size, self.features, self.features]
        )
        ...
        householder_VVT = (
            torch.transpose(householder_Vt_matr, dim0=-2, dim1=-1) @ householder_Vt_matr
        )
    ...
    def test_forward(self):
        ...
        for i in range(self.batch_size):
            ...
            output_ref = (
                householder_U_matr[i]
                @ (diag_entries_S[i] * (householder_Vt_matr[i] @ self.random_input[i]))
                + bias[i]
            )
    ...
    def test_inverse(self):
        ...
        for i in range(self.batch_size):
            ...
            Spinv_UtX_ref = torch.diag(1.0 / diag_entries_S[i]) @ UtX_ref
