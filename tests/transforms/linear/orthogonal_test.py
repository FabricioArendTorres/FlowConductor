...
from flowcon.transforms.linear import orthogonal
...
class BatchwiseParameterizedHouseholderTest(TransformTest):
    ...
    def test_forward(self):
        transform = orthogonal.ParametrizedHouseHolder(self.batch_q_vectors)
    ...
    def test_forward_inverse_are_consistent(self):
        ...
        batch_q_vectors = torch.randn(self.batch_size, self.features)
...
class ParameterizedHouseholderTest(TransformTest):
    def setUp(self):
        ...
        torch.manual_seed(self.seed)
        self.ref_householder = orthogonal.OrthogonalHouseholder(self.features)
        self.ref_householder.q_vectors = torch.nn.Parameter(
            torch.randn_like(self.ref_householder.q_vectors)
        )
...
class CaleyTest(TransformTest):
    def test_forward(self):
        features = 10
        batch_size = 50

        for features in [2, 11, 12]:
            # for num_transforms in [11]:
            with self.subTest(features=features):
                transform = orthogonal.OrthogonalCaley(n_features=features)
    ...
    def test_matrix(self):
        for features in [2, 6, 11]:
            with self.subTest(features=features):
                ...
                det_ref = torch.tensor(1.0)
                torch.testing.assert_close(torch.abs(matrix.det()), det_ref)

    def test_matrix_after_change(self):
        """
        Ensure that this is really a parametrization of orthogonal matrices,
        not just some projection of the unconstrained values onto orthogonal matrices..
        """
        for features in [2, 6, 11]:
            with self.subTest(features=features):
                transform = orthogonal.OrthogonalCaley(n_features=features)
                transform.orth_linear.parametrizations.weight.original.data += 2 * (
    ...
    def test_forward_inverse_are_consistent(self):
        ...
        for num_features in [2, 11, 12]:
            with self.subTest(num_features=num_features):
                inputs = torch.randn(batch_size, num_features)

                transform = orthogonal.OrthogonalCaley(n_features=num_features)

                self.eps = 1e-5
                self.assert_forward_inverse_are_consistent(transform, inputs)


class OrthogonalHouseholderTest(TransformTest):
    def test_forward(self):
        features = 10
        batch_size = 50
        ...
        for features in [2, 11, 12]:
            # for num_transforms in [11]:
            with self.subTest(features=features):
                transform = orthogonal.OrthogonalHouseholder(n_features=features)
                matrix = transform.matrix()
                inputs = torch.randn(batch_size, features)
                outputs, logabsdet = transform(inputs)
                self.assert_tensor_is_good(outputs, [batch_size, features])
    ...
    def test_matrix_after_change(self):
        ...
        for features in [2, 6, 11]:
            with self.subTest(features=features):
                transform = orthogonal.OrthogonalHouseholder(n_features=features)
                transform.q_vectors.data += 2 * (torch.rand(features, features))
    ...
    def test_forward_inverse_are_consistent(self):
        ...
        for num_features in [2, 11, 12]:
            with self.subTest(num_features=num_features):
                ...
                transform = orthogonal.OrthogonalHouseholder(n_features=num_features)

                self.eps = 1e-5
                self.assert_forward_inverse_are_consistent(transform, inputs)


class OrthogonalHouseholderGEQRTest(TransformTest):
    transform = orthogonal.OrthogonalHouseholderGEQR
    ...
    def test_forward(self):
        ...
        for features in [2, 11, 12]:
            # for num_transforms in [11]:
            with self.subTest(features=features):
                transform = self.transform(n_features=features)
                matrix = transform.matrix()
                inputs = torch.randn(batch_size, features)
                outputs, logabsdet = transform(inputs)
                self.assert_tensor_is_good(outputs, [batch_size, features])
    ...
    def test_matrix_after_change(self):
        ...
        for features in [2, 6, 11]:
            with self.subTest(features=features):
                transform = self.transform(n_features=features)
                transform._A.data += 2 * (torch.rand(features, features))

                matrix = transform.matrix()
                self.assert_tensor_is_good(matrix, [features, features])
                self.eps = 1e-5  # type: ignore
                self.assertEqual(matrix @ matrix.t(), torch.eye(features, features))
                self.assertEqual(matrix.t() @ matrix, torch.eye(features, features))
                self.assertEqual(matrix.t(), torch.inverse(matrix))
                det_ref = torch.tensor(1.0)
    ...
    def test_forward_inverse_are_consistent(self):
        ...
        for num_features in [2, 11, 12]:
            with self.subTest(num_features=num_features):
                ...
                transform = self.transform(n_features=num_features)
