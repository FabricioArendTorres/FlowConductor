...
class AdaptiveSigmoidTest(TransformTest):
    def setUp(self):
        ...
        self.transform = SumOfSigmoids(
            features=self.features, n_sigmoids=self.n_sigmoids, iterations_bisection_inverse=30
        )
        self.eps = 1e-5
    ...
    def test_new_parameterized(self):
        ...
        transform_parametrized = SumOfSigmoids(
            features=self.features, n_sigmoids=self.n_sigmoids, raw_params=raw_params
        )
        outputs, logabsdet = self.transform.forward(self.inputs)
