...
from enflows.transforms import SumOfSigmoids
...
class AdaptiveSigmoidTest(TransformTest):
    ...
    def test_new_parameterized(self):
        ...
        transform_parametrized = SumOfSigmoids(features=self.features, n_sigmoids=self.n_sigmoids, raw_params=raw_params)
        outputs, logabsdet = self.transform.forward(self.inputs)
