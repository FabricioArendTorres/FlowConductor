...
@parameterized_class(
    ("batch_size", "matrix_dim", "diag_transform"),
    [
        (10, 2, Exp()),
...
class TransformDiagonalTest(TransformTest):
    def setUp(self):
        self._inputs = torch.randn(
            self.batch_size, self.matrix_dim * self.matrix_dim
        ).requires_grad_(True)
        ...
        if self.diag_transform == "exp":
            ...
            self.transform = TransformDiagonal(
                N=self.matrix_dim, diag_transformation=self.diag_transform
            )
        self.eps = 2e-5
    ...
    def test_forward(self):
        ...
        logabsdet_ref = torchutils.logabsdet(
            torchutils.batch_jacobian(outputs.view(self.batch_size, -1), self._inputs)
        )
        self.assertEqual(logabsdet, logabsdet_ref)

        self.assertGreater(
            torch.diagonal(outputs, dim1=-2, dim2=-1).detach(),
    ...
    def test_inverse(self):
        ...
        inputs_rec, logabsdet_inv = self.transform.inverse(
