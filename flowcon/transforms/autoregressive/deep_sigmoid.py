...
from flowcon.transforms.monotonic.adaptive_sigmoids import DeepSigmoidModule
...
class MaskedDeepSigmoidTransform(AutoregressiveTransform):
    ...
    def __init__(
        ...
        super().__init__(made)
        self.deep_sigmoid_module = self.DeepSigmoidMadeModule(
            n_sigmoids=n_sigmoids, eps=3e-5, num_inverse_iterations=50
        )
    ...
    def _elementwise_forward(self, inputs, autoregressive_params):
        ds_params = autoregressive_params.view(
            inputs.shape[0], self.features, self._output_dim_multiplier()
        )
        outputs, logabsdet = self.deep_sigmoid_module.forward_given_params(
            inputs, dsparams=ds_params / 5
        )
