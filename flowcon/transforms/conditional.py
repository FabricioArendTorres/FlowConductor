...
class ConditionalTransform(Transform):
    ...
    def __init__(
        ...
        if conditional_net is not None:
            ...
            self.conditional_net = self.set_default_network(
                activation,
                context_features,
                dropout_probability,
                hidden_features,
...
class ConditionalShiftTransform(ConditionalTransform):
    ...
    def _forward_given_params(self, inputs, conditional_params):
        shift = self._unconstrained_shift(conditional_params)
        # scale = torch.sigmoid(unconstrained_scale + 2.0) + self._epsilon
    ...
    def _unconstrained_shift(self, conditional_params):
        ...
        # return unconstrained_scale, shift
        conditional_params = conditional_params.view(-1, self.features)
...
class ConditionalScaleTransform(ConditionalTransform):
    ...
    def _forward_given_params(self, inputs, conditional_params):
        scale = self._constrained_scale(conditional_params)
...
class ConditionalLUTransform(ConditionalTransform):
    def __init__(
        ...
        self.default_pivot = torch.nn.Parameter(
    ...
    def _inverse_given_params(self, inputs, conditional_params):
        lower, upper = self._create_lower_upper(conditional_params)
        outputs = torch.linalg.lu_solve(
            torch.tril(lower, -1) + upper,
    ...
    def _create_lower_upper(self, conditional_params):
        ...
        lower = (
            torch.nn.functional.softplus(self.scale_non_diag)
            * torch.tril(unconstrained_matrix_vals, diagonal=-1)
            + self.diag_entries
        )
        upper_diag = torch.diag_embed(
            self.softplus(unconstrained_matrix_vals.diagonal(0, -1, -2)) + self.eps
        )
        upper = (
            torch.nn.functional.softplus(self.scale_non_diag)
            * torch.triu(unconstrained_matrix_vals, diagonal=1)
            + upper_diag
        )
        return lower, upper
...
class ConditionalOrthogonalTransform(ConditionalTransform):
    ...
    def _get_matrices(self, conditional_params) -> ParametrizedHouseHolder:
        q_vectors = self._unconstrained_entries(conditional_params)
...
class ConditionalSVDTransform(ConditionalTransform):
    ...
    def _unconstrained_params(self, conditional_params):
        ...
        if self.use_bias:
            q_vectors_U, q_vectors_Vt, diag_entries_S, bias = torch.split(
                conditional_params, output_shapes, -1
            )
        else:
            q_vectors_U, q_vectors_Vt, diag_entries_S = torch.split(
                conditional_params, output_shapes[:-1], -1
            )
            bias = None

        return (
            q_vectors_U.view(-1, self.features, self.features),
            q_vectors_Vt.view(-1, self.features, self.features),
            diag_entries_S,
            bias,
        )
...
class ConditionalUMNNTransform(ConditionalTransform):
    ...
    def _forward_given_params(self, inputs, conditional_params):
        z, jac = self.transformer(
            inputs, conditional_params.reshape(inputs.shape[0], inputs.shape[1], -1)
        )
    ...
    def _inverse_given_params(self, inputs, conditional_params):
        ...
        z, jac = self.transformer(
            x, conditional_params.reshape(inputs.shape[0], inputs.shape[1], -1)
        )
        log_det_jac = -jac.log().sum(1)
...
class ConditionalPlanarTransform(ConditionalTransform):
    def __init__(
        ...
        self.softplus = torch.nn.Softplus()
        self.diag_entries = torch.nn.Parameter(
            torch.eye(features, device="cuda"), requires_grad=False
        )
    ...
    def _forward_given_params(self, inputs, conditional_params):
        u_, w_, b_ = self._create_uwb(conditional_params)
        pre_act = (
            torch.bmm(
                inputs.view(-1, 1, self.features), torch.transpose(w_, dim0=-2, dim1=-1)
            ).squeeze()
            + b_.squeeze()
        )
...
class ConditionalSylvesterTransform(ConditionalTransform):
    def __init__(
        ...
        self.reverse_idx = torch.nn.Parameter(
        ...
        self.identity = torch.nn.Parameter(
            torch.eye(features, features).unsqueeze(0), requires_grad=False
        )
    ...
    def _forward_given_params(self, inputs, conditional_params):
        R1, R2, Q, bias = self._create_mats(conditional_params)
    ...
    def _create_mats(
        ...
        R_full, R2_diag, q_unconstrained, b = torch.split(
            conditional_params, self.__output_splits, -1
        )

        R1, R2 = self._create_upper(
            R_full.view(-1, self.features, self.features), R2_diag, self.triu_mask
        )
    ...
    def build_orthogonal_matrix(self, q_vectors: torch.Tensor):
        ...
        outputs1 = batchwise_householder_transforms_nodet(
            self.identity[:, 0, :], q_vectors[:, self.reverse_idx, :]
        )
        outputs2 = batchwise_householder_transforms_nodet(
            self.identity[:, 1, :], q_vectors[:, self.reverse_idx, :]
        )
    ...
    def _create_upper(full_matr_r, diag_vals, triu_mask) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        diag_1 = torch.diag_embed(
            torch.tanh(torch.diagonal(full_matr_r, dim1=-2, dim2=-1)), dim1=-2, dim2=-1
        )
