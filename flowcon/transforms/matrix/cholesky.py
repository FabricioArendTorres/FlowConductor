...
class CholeskyOuterProduct(Transform):
    ...
    def forward(self, inputs, context=None):
        ...
        logabsdet = self.N * np.log(2.0) + (self.powers * diagonal.log()).sum(-1)
    ...
    def check_pos_low_triang(self, inputs):
        assert inputs.shape[-2] == inputs.shape[-1], (
            "input tensor must be mini batch of square matrices"
        )
        upper_indices = np.triu_indices(self.N, k=1)
        assert torch.all(inputs[:, upper_indices[0], upper_indices[1]] == 0.0), (
